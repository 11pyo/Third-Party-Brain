...
## 2026-09-01
- `[ALGO]` **동기화 점검기에 「세션 착수 인덱스 신선도」 점검 추가 — 규칙으로 남겨둔 마지막 수기 단계를 도구로 옮김.** 2026-08-25에 `tasks-index.md` 생성기를 도입하면서 "카드를 고쳤으면 인덱스를 재생성한다"는 **규칙**만 남겨 뒀는데, 정기 감사(AI 규칙 총점검) 실측에서 그 규칙이 실제로 새어 나간 상태를 발견했다(인덱스가 정본보다 22분 낡은 채 방치 — 표기 761,826자/2,641줄 vs 실제 764,764자/2,643줄). 같은 파일군의 보드 동기화는 이미 점검기가 잡고 있었으므로 인덱스도 같은 게이트로 흡수했다. `check_index_freshness()` 신설: ① 인덱스 부재 ② `mtime(index) + 2s < mtime(tasks.md)` ③ 인덱스 머리말의 "`tasks.md` N자 / M줄" 표기와 실제값 대조 — 어긋나면 재생성 명령을 붙여 실패(종료코드 1)로 보고.
  - ⚠️ 구현 함정: 글자수 정의를 생성기와 **반드시 일치**시켜야 한다(생성기는 줄 분할 후 `sum(len(l))` = **개행 제외** 글자수, 줄수는 `개행수+1`). 처음에 전체 문자열 길이로 세었다가 항상 불일치로 뜨는 거짓 실패를 만들었고 실측 대조로 잡았다.
  - 영향: `task-dashboard/_check_board_sync.py`(상수 `INDEX_MD`·`INDEX_SIZE_RE`·`MTIME_SLACK` + `check_index_freshness()` + `main()` 1줄), 공개 동작본 `ai-collab-dashboard/check-board-sync.py` 동일 이식. 기존 보드 대조 로직·출력 형식은 **무변경**(추가만).
  - 검증: 낡은 인덱스 상태에서 실행 → 어긋남 2건·종료코드 1 / 생성기 재실행 후 → 어긋남 0건·종료코드 0. 기존 참고(ℹ️) 출력 7건 동일 유지.
  - 이유: 정기 감사에서 "규칙만 있고 도구가 없는 단계는 반드시 낡는다"가 또 실증됨 — 2026-08-25 `[ALGO]`(보드 동기화)와 같은 처방을 인덱스에 적용.
- `[DOC]` **같은 감사에서 문서 드리프트 정정**(프로그램 무관, 기록용): 원본 아카이브의 `archive-structure.md` 트러블슈팅 표에 아티클 1건의 행이 누락돼 있었다 — **카운트(53)는 맞는데 행만 52개**라 겉으로는 정상으로 보였다. 그 밖에 규칙 문서에 하드코딩된 파일 크기, 폐지된 「칸반 요약 표」 잔존 서술을 정정.
  - **교훈(복제 시 유효): 카운트 일치는 목록 일치를 보장하지 않는다.** 색인 점검은 합계가 아니라 **id 전수 대조**로 할 것(`<article id>` 집합 ↔ 인덱스 표 집합).

## 2026-08-25
- `[STRUCTURE]` **대시보드 정본(`tasks.md`) 경량 인덱스 자동생성 도입 — 세션 착수 규칙("먼저 tasks.md 정독") 물리적 실행 불가 해소.** `tasks.md`가 656,771자/2,250줄로 자라며 "새 세션은 착수 시 tasks.md를 먼저 읽는다"는 운영 규칙이 한 컨텍스트에 안 들어가 실행 불가능해짐 → `_gen_tasks_index.py`(신규) 도입: 살아있는 카드 목록 + **카드별 tasks.md 라인 범위**만 담은 `tasks-index.md`를 생성(656,771자→5,018자, 약 1/130). 세션은 인덱스만 읽고 필요한 카드만 그 라인 범위로 부분 읽기(`sed -n 'A,Bp'`). 카드 메타(상태·요청자·기한·다음액션) 추출 시 출처 우선순위를 **보드 `task-board.html`의 `const TASKS` 배열 > tasks.md 상단 칸반 요약 표**로 고정 — 손 관리 요약 표가 실측상 가장 먼저 낡음을 확인(사례: 어떤 카드는 카드 본문·보드 배열 둘 다 최신 상태로 갱신됐는데 요약 표만 옛 상태로 잔존). 이 결정에 맞춰 `tasks.md` 상단 「📊 한눈에 보기(칸반 요약)」 수기 표를 걷어내는 작업 착수(진행 중) — 카드 데이터 3중 수기입력(본문/요약표/보드배열) 중 가장 잘 낡는 층을 제거하고 메타 단일 출처를 보드 배열로 정리.
  - 영향: `task-dashboard/_gen_tasks_index.py`(신규), `task-dashboard/tasks-index.md`(생성물), `task-dashboard/tasks.md`(상단 칸반 요약 표 제거 진행 중), `07-dashboard-operating-guide.human.md`(B절의 "칸반 요약 표" 전제 서술을 이 변경에 맞게 정정 — 별도 diff).
  - 이유: 카드 수 증가로 정본 파일이 세션 컨텍스트 예산을 초과 — 사람이 갱신할 목록을 늘리지 않는 방식(탐색 층 INV10과 같은 정신)으로 해결.
- `[ALGO]` **대시보드 3중 데이터 동기화 점검기 신설 — `tasks.md` ↔ `task-board.html`(`const TASKS`) 크로스체크.** 카드 데이터를 본문/칸반 요약 표/보드 배열 3벌로 수기 이중입력해 한쪽만 갱신되는 드리프트가 반복(보드에서 카드가 누락된 전례 다수) — 메모리·규칙 문서로 막아왔으나 규칙 자체가 시간이 지나며 잊히는 문제. `_check_board_sync.py`(신규): 세 벌을 교차 대조해 누락·잔재·상태 불일치를 보고하고, 어긋나면 종료코드 1(고치지는 않음 — 무엇이 사실인지는 사람 판단). 첫 실행에서 실제 불일치 15건 검출. 구현: `const TASKS` 배열 탐지를 단순 문자열 검색으로 하면 카드 ailog 본문의 산문 언급(예: "…const TASKS 배열…")에 먼저 걸리는 문제를 실제로 겪음 → 선언부는 정규식 `const\s+TASKS\s*=\s*\[`로 특정하고, 문자열 리터럴 내부의 괄호를 무시하는 스캐너로 배열 구간만 잘라낸 뒤 `json.loads`로 파싱. 이 배열은 보드 저장 기능이 `JSON.stringify`로 재작성하므로 **키까지 따옴표 붙은 정식 JSON**이다.
  - 영향: `task-dashboard/_check_board_sync.py`(신규). 핵심 아카이브 도구(`archive-server.py` 등) 무관.
  - 검증: 실제 사내 대시보드 데이터로 1회 실행 — 불일치 15건 검출(상세 내용은 사내 카드 식별정보 포함이라 이 CHANGELOG엔 비기재).
  - 이유: 반복되는 3중 수기입력 드리프트를 사람이 매번 눈으로 대조하지 않고 기계로 표면화.

## 2026-08-20
- `[STRUCTURE]` **탐색 층(Navigator) 도입 — 정본을 쪼개지 않고 다중 페이지처럼 탐색.** 아티클 110개에 이르자 롱페이지 단일 화면으로는 사람이 자력으로 못 찾는 문제(사용자 제기: "AI 없이 혼자 보기엔 번잡"). 파일 분할(위키형)은 **수기 이중 유지 → 드리프트 + AI가 정본 확인에 왕복**이라는 더 큰 비용을 만들므로 기각. 대신 **정본 1벌 + 보기 층 3모드**로 해결: `mode-hub`(색인) / `mode-doc`(한 편만, `:target` 기반이라 딥링크·뒤로가기 유지) / `mode-all`(기존 롱페이지, 검색 시 자동 전환). 사이드바는 카테고리 접기 + 제목 빠른찾기 + 현재 글 하이라이트.
  - **허브 색인 5탭은 전부 런타임 생성**(케이스 카드 / T-Code / 테이블 / 주제·분류 / 최근). 파일에 기록하지 않으므로 아티클을 추가해도 **사람이 갱신할 목록이 0개** — 인덱스형(링크목록 수기관리)의 유실 약점을 자동화로 제거. 실측 산출: T-Code 195 · 테이블 117 · 케이스 50 · 주제 12클러스터.
  - **주제 매칭은 제목 + 태그칩(`.tags .tag`)** 사용. `data-tags`(AI 검색용 키워드 덤프)로 매칭하면 한 주제가 전체 절반을 삼켜 색인 구실을 못 함(실측 매출품의 62/110 → 33/110로 교정).
  - 영향: `archive.html`(`</body>` 직전 `<style>`+`<script>` 단일 블록 추가 — **기존 마크업 라인 번호 불변** → `archive-structure.md` 재계산 불요 / `saveFile()` export 정리 루틴에 탐색층 클래스·인라인 display strip 2줄 추가), `reference-implementation/archive/archive.html`(공개 동작본 동일 이식), `archive-structure.md`(탐색 층 불변식 절 추가).
  - 부수 수정 2건: ① **`saveFile()` 잠재 결함** — 검색 중(`style.display:none`) 저장하면 인라인 display가 저장본에 박히던 문제를 정리 루틴에 편입. ② **HTML 결함** — 아티클 본문에서 `</strong>`을 `</a>`로 잘못 닫아 **이후 아티클 19개가 `<strong>` 안에 중첩**되어 있던 것을 발견·수정(탐색 층이 자식 선택자로 아티클을 세다가 91/110 불일치로 드러남). 이후 탐색 층 선택자도 자식결합자(`>`) 대신 후손 선택자로 변경해 재발 시에도 무해하도록 방어.
  - 검증: 브라우저 실측 — 5탭 렌더·모드 전환·딥링크 복원·검색 자동전환·사이드바 필터/접기 정상, 콘솔 오류 0, 가로 스크롤 없음. **export 시뮬레이션**(문서모드+검색 켠 상태에서 저장 루틴 적용) 결과 주입요소 0·뷰클래스 0·인라인style 0·아티클 110/사이드바 링크 110 보존.
  - 이유: 사용자 요청("노션처럼 페이지를 구조화하고 싶다. 단 AI 작업 속도를 해치지 않는 방법으로").
...
- `[DEPLOY]` **file:// 저장 안내 정정 — 원본 직접쓰기는 localhost/https 전용, file://은 다운로드뿐.** 실사용 보고: `file:///…/archive.html`(더블클릭)에서 💾 후 새로고침 시 경고 배너, 원본 미반영. 디스크 증거: 저장 시각 이후 C:프로필+D: 전체에 수정된 html 0건(원본 mtime=직전 편집 그대로) = file://에선 **쓰기가 어디에도 안 됨**. 원인: File System Access(`showSaveFilePicker`/`createWritable`)는 **opaque origin인 file://에서 불가** → 다운로드 폴백뿐(그 다운로드도 OS Save-As 취소 시 무파일). 종전 인앱 문구가 "더블클릭이 가장 깔끔"·"Chrome·Edge는 파일에 바로 저장"이라 **오해 유발**. 수정: 로드 시 `!showSaveFilePicker || location.protocol==='file:'`이면 💾버튼을 **"💾 다운로드"** 로 재라벨 + 힌트에 **"원본 바로저장은 `http://localhost`(로컬 서버)로 열라"** 명시(아카이브 검색서버 `archive-server.py`가 이미 localhost:5174 제공).
  - 영향: 정본 `archive.html`(비공개) + `reference-implementation/archive/archive.html`(인앱 힌트·버튼 라벨·타이틀). 코드 로직(saveFile 자체)은 불변 — 라벨/안내만. 대시보드들도 동일 제약(README가 "더블클릭" 안내) → 같은 localhost 가이드 적용 대상(후속).
  - 검증: http(preview)에선 버튼 "💾 저장" 유지·힌트 localhost 노트 포함·아티클수 보존·console 0(정본 70·샘플 9). file:// 분기는 조건상 "💾 다운로드"로 재라벨.
  - 교훈(INV 후보): **"서버 없이 file://로 연다"는 읽기엔 맞지만 *원본 저장*엔 거짓** — 인플레이스 쓰기는 보안컨텍스트(secure + non-opaque origin) 필요. 무서버 저장의 진짜 형태는 "다운로드→수동 교체"이고, 원클릭 인플레이스는 localhost(=.bat 더블클릭)다. 안내문이 이 경계를 흐리면 사용자는 "저장됐다"고 오인.
